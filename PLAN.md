...
| --- | --- | --- | --- |
| `urnway-api` | Repo exists, builds, has live auth/session flow, serves docs at `/` and `/docs`, exposes Mezo-backed BTC/MUSD reads, supports direct send plus payment links and QR-backed direct requests through preflight, submit, stale/reset, and Goldsky confirmation, stores travel savings goals plus trips with itinerary items, AI draft generation, and expense tracking, and now adds booking-first flight search/book plus ticket issuance and boarding-pass routes | P0 | Wallet activity indexing, nearby hardening, native vault integration, provider-backed travel inventory, and most route families are still missing |
...
| `urnway-mobile` | Repo exists with onboarding/auth flow, SecureStore session handling, a real balance-backed Home dashboard, a Pay tab that now supports direct send, links, QR requests, and nearby BLE broadcasts plus scanner routes, a Save launchpad for Mezo-backed travel goals, and a Trips surface that is now booking-first with flight search, booking detail, ticket issuing, boarding-pass detail, plus secondary planning/itinerary/spend flows | P1 | Nearby hardening is still thin, and broader product screens still depend on more API routes |
