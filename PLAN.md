...
| --- | --- | --- | --- |
| `urnway-api` | Repo exists, builds, has live auth/session flow, serves docs at `/` and `/docs`, exposes Mezo-backed BTC/MUSD reads, now includes Urnway-held MUSD balance accounts plus top-up, send checkout, and booking checkout routes, supports direct send plus payment links, QR-backed direct requests, and nearby BLE flows, stores travel savings goals plus trips with itinerary items, AI draft generation, expense tracking, booking-first travel routes, and a protected places autocomplete route for flight, stay, and trip destination suggestions | P0 | Wallet activity indexing, nearby device smoke testing, native vault integration, real provider payment settlement, and most route families are still missing |
...
| `urnway-mobile` | Repo exists with onboarding/auth flow, SecureStore session handling, a real balance-backed Home dashboard, a Pay tab that now supports Urnway balance top-up, balance-aware direct send, links, QR requests, and nearby BLE discoverable send with PAY/ACK/DONE messaging backed by send checkouts, a Save launchpad for Mezo-backed travel goals, and a Trips surface that is now booking-first with provider-backed search plus balance-aware booking checkout, boarding-pass flows, and shared location/date pickers for search and manual trip planning | P1 | Nearby still needs full device smoke testing and native rebuild validation, live hotel-provider validation still depends on liteAPI credentials and payment setup, and broader product screens still depend on more API routes |
